...
def estimate_class_covariances(X, y):
    ...
    sample_covariances = np.stack([(np.matmul((X[y == c + 1] - sample_means[c]).T, X[y == c + 1] - sample_means[c])) / X[y == c + 1].shape[0] for c in range(K)])
...
def calculate_score_values(X, class_means, class_covariances, class_priors):
    ...
    score_values = np.stack([[- (D / 2) * np.log(2 * math.pi) - 0.5 * np.log(np.linalg.det(class_covariances[c])) 
                              - 0.5 * np.matmul(np.matmul((X[i] - class_means[c]), np.linalg.inv(class_covariances[c])), (X[i] - class_means[c]).T) 
...
def estimate_shared_class_covariance(X, y):
    ...
    sharedMean = np.mean(X)

    sample_covariances = np.stack([np.matmul((X - sharedMean).T, X - sharedMean) / X.shape[0] for c in range(K)])
    # your implementation ends above
    return(sample_covariances)
